...
app.get('/translate/vie-en', (req, res) => {
  try {
    ...
    results = results.filter(item => item.vietnamese.toLowerCase().match(keySearch))
    if (!results.length) {
...
app.post('/add-word', (req, res) => {
  try {
    ...
    if (!vietnamese || !english) {
      throw new Error(`Hãy nhập cụm từ điển với key là 'vietnamese' và 'english' !`)
    }
    db.get('words')
      .push({ vietnamese, english })
      .write()
    res.status(201).json({
      success: true,
